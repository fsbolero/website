---
title: Hosting models (legacy)
subtitle: WebAssembly or server-side
---

> Note: Blazor 8 and Bolero 0.24 introduced new hosting models, [documented here](Hosting).
> This page describes the pre-Blazor 8 hosting models, which are still supported but considered legacy.
...
Or with the Bolero project template older than v0.24:
...
```sh
dotnet new bolero-app --render=LegacyWebAssembly
```
...
To create a server-side Bolero app, use the [dotnet project template](index#creating-a-project) with the following arguments:

```sh
dotnet new bolero-app --render=LegacyServer
```
